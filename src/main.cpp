...
class Model: public MessagePassing
{
    public:
        ...
        Model(size_t num_atoms, size_t embed_size, size_t out_channels, size_t p, bool bias, size_t n_layers=1): MessagePassing(){ 
            register_module("embed", new Embedding(num_atoms, embed_size)); // num_aa * embed_size for protein A and B
            register_module("pre", new MLP(embed_size, {embed_size*2, embed_size}, bias, p)); //preprocessing => num_aa * embed_size for protein A and B
            ...
            }; // => num_aa * embed_size for proteins A and B
