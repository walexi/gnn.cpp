...
namespace nn
{
    ...
    public:
        ...
        void register_parameter(std::string name, std::shared_ptr<cyg::tensor<float>> p) { _parameters[name] = p; }
        void zero_grad()
        {
        ...
        std::shared_ptr<cyg::tensor<float>> get_parameter(std::string name)
        {
            if (_parameters.find(name) == _parameters.end())
                throw std::runtime_error("invalid input, no parameter with given name");
            return _parameters[name];
        }
        std::shared_ptr<cyg::tensor<float>> operator()(const std::shared_ptr<cyg::tensor<float>> &input_tensor) { return forward(input_tensor); }; // you prolly dont wanna pass in a tensor without grad enabled
        ...
        std::map<std::string, std::shared_ptr<cyg::tensor<float>>> parameters() const { return _parameters; }
        ~Module()
        {
            _parameters.clear();
            _modules.clear();
    ...
    protected:
        ...
        std::map<std::string, std::shared_ptr<cyg::tensor<float>>> _parameters;
    };
    ...
    public:
        std::shared_ptr<cyg::tensor<float>> weight;
        std::shared_ptr<cyg::tensor<float>> bias = nullptr;
        ...
        Linear(size_t in_features, size_t out_features, bool bias = true) : Module(), _bias(bias), _in_features(in_features), _out_features(out_features)
        {
            ...
            weight = make_shared<cyg::tensor<float>>(weight_feat, 1, true, true);
            register_parameter("weight", weight);
            if (bias)
            {
                this->bias = make_shared<cyg::tensor<float>>(bias_feat, 1, true, true);
    ...
    class BatchNorm : public Module{
        ...
        public:
            ...
            std::shared_ptr<cyg::tensor<float>> forward(const std::shared_ptr<cyg::tensor<float>>& x){
                ...
                if(_tracking_running_stats && training) {
                    mean->enable_grad(false);  //temporaily disable autograd to compute stats below, not neccessary though
                    auto x_clone = x->clone(false);
        ...
        std::shared_ptr<cyg::tensor<float>> _mean_avg;
        std::shared_ptr<cyg::tensor<float>> _var_avg;
        int _num_features;
        float _eps;
        double _momentum;
        bool _affine;
